...
market_reason = ops.get("market_explanation", {})
if market_reason:
    st.caption(market_reason.get("summary", ""))
    with st.expander("为什么是这个市场温度", expanded=False):
        for item in market_reason.get("items", []):
            st.write(f"- {item}")
...
st.subheader("今日结论")
conclusion_lines = ops.get("today_conclusion") or [ops.get("one_liner", "主线数据不足，先观察数据源状态。")]
for item in conclusion_lines:
    st.info(item)
with st.expander("为什么今天这样判断", expanded=True):
    st.write(ops.get("why_today", "主线数据不足，先观察数据源状态。"))
...
for col, label in zip(action_cols, ["重点研究", "等回调", "只观察 / 不追", "回避"], strict=False):
    with col:
        ...
        for row in rows:
            ...
            st.caption(
                f"{row.get('explanation') or row.get('reason', '')} | 综合 {row.get('score', '')} / "
